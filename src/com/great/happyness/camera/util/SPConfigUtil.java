...
public class SPConfigUtil {
	...
	synchronized public static String load(final String key) {
		String cf = null;
		if (WiseApplication.CONTEXT == null) {
			return cf;
		}
		try {
			SharedPreferences sp = WiseApplication.CONTEXT.getSharedPreferences(SP_NAME, Context.MODE_PRIVATE);
	...
	synchronized public static void save(final String key, final String val) {
		if (WiseApplication.CONTEXT == null || val == null) {
			return;
		}
		try {
			SharedPreferences sp = WiseApplication.CONTEXT.getSharedPreferences(SP_NAME, Context.MODE_PRIVATE);
