...
//    }
    ...
    public static Bitmap stringToBitmap(String text, int size, int color, Typeface font, int bgColor) {
        ...
        p.setTextSize(TypedValue.applyDimension(
                ...
                WiseApplication.CONTEXT.getResources().getDisplayMetrics()));

        float space =  WiseApplication.CONTEXT.getResources().getDimensionPixelSize(R.dimen.px_2);  //背景padding

        int textHeight = (int) getTextHeight(p);    //字的高度
        int pixLen = (int) (getFontHeight(p) + 2* space);//正方形边长

        int offsetY =   WiseApplication.CONTEXT.getResources().getDimensionPixelSize(R.dimen.px_2);    //y方向上的偏移  手动调整
    ...
    public static float getLineSpaceHeight(){
        return   WiseApplication.CONTEXT.getResources().getDimensionPixelSize(R.dimen.px_4);
    }
    ...
    public static float getCenterY(Paint paint){
        int offset =  WiseApplication.CONTEXT.getResources().getDimensionPixelSize(R.dimen.px_2);
